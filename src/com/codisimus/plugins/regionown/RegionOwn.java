...
public class RegionOwn extends JavaPlugin {
    ...
    private static String loadValue(String key) {
        ...
        if (!p.containsKey(key)) {
            System.err.println("[RegionOwn] Missing value for "+key+" in config file");
    ...
    public static void saveRegion(Region region) {
        try {
            ...
            p.setProperty("BlockPvP", String.valueOf(region.blockPvP));
            p.setProperty("BlockPvE", String.valueOf(region.blockExplosions));
            p.setProperty("BlockExplosions", String.valueOf(region.blockExplosions));
            p.setProperty("LockChests", String.valueOf(region.lockChests));
            p.setProperty("LockDoors", String.valueOf(region.lockDoors));
            p.setProperty("DisableButtons", String.valueOf(region.disableButtons));
            p.setProperty("DisablePistons", String.valueOf(region.disablePistons));
            p.setProperty("AlarmSystem", String.valueOf(region.alarm));
            p.setProperty("RegenerateHealth", String.valueOf(region.heal));
            p.setProperty("RegenerateHunger", String.valueOf(region.feed));

            //Write the Region Properties to file
            FileOutputStream fos = new FileOutputStream(dataFolder+"/Regions/"+region.name+".properties");
            p.store(fos, null);
            fos.close();
    ...
    static void saveRegionOwner(RegionOwner owner) {
        try {
            ...
            FileOutputStream fos = new FileOutputStream(dataFolder+"/RegionOwners/"+owner.name+".properties");
            p.store(fos, null);
            fos.close();
        }
        catch (Exception saveFailed) {
    ...
    public static Region findRegion(String name) {
        if (ownedRegions.containsKey(name))
            return ownedRegions.get(name);
        else if (regions.containsKey(name))
    	    ...
    	    public void run() {
                ...
                for (String key: lastDaySeen.stringPropertyNames())
                    if (Integer.parseInt(lastDaySeen.getProperty(key)) < cutoffDay) {
                        System.out.println("[RegionOwn] Clearing Regions that are owned by "+key);
